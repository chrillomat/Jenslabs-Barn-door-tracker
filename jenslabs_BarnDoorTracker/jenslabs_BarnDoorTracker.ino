...
static const float THREADS_PER_CM = 10;   // Number of threads in rod per cm of length
static const float BASE_LEN_CM = 19.9;   // Length from hinge to center of rod in cm
...
static const int autoTrackerActivePin = 8;  // Arduino analogue pin connected to sidereal mode switch
//static const int manualMovementPin= 9; // Arduino analogue pin connected to highspeed mode switch
static const int directionPin = 7; // Arduino analogue pin connected to direction
static const int motorOnOffPin = 10;
...
boolean firstPassAfterStop = true;

// Global initialization when first turned off
void setup(void)
{
    pinMode(autoTrackerActivePin, INPUT_PULLUP);
 //   pinMode(pinInHighspeed, INPUT_PULLUP);
    pinMode(directionPin, INPUT_PULLUP);
    pinMode(motorOnOffPin, INPUT_PULLUP);
...
void loop() {  
    ...
    if (digitalRead(autoTrackerActivePin) == LOW && digitalRead(motorOnOffPin)==LOW) {
        if (firstPassAfterStop==true){
          state_sidereal_enter();
          }
        state_sidereal_update();
        //barndoor.transitionTo(stateSidereal);
    } else if (digitalRead(autoTrackerActivePin) == HIGH && digitalRead(motorOnOffPin)==LOW) {
        state_highspeed_update();
    } else {
        motor.stop();
    }
